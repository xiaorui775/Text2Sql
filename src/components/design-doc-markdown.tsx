  ...
  return (
    <div className={`prose prose-slate dark:prose-invert max-w-none text-sm leading-7 ${className}`}>
      <ReactMarkdown
        ...
        components={{
          table: ({ children }) => (
            <div className="my-6 w-full overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
              <table className="w-full table-fixed border-collapse bg-white dark:bg-slate-900 text-xs sm:text-sm">{children}</table>
            </div>
          ),
